...
//!
use crate::{AccountId, Call, Fee, Fees, Runtime};
use fees::{AssetOf, CheckCallFee, Trait};
use runtime_primitives::traits::{Applyable, As, Block as BlockT, Checkable, Zero};
use support::{
...
impl<Block, Context, T>
    ChargeExtrinsicFee<AccountId, <Block::Extrinsic as Checkable<Context>>::Checked>
    for ExtrinsicFeeCharger<Block, Context, T>
where
    T: Trait<Fee = Fee>,
    Context: Default,
    ...
    <Block::Extrinsic as Checkable<Context>>::Checked:
        Applyable<Index = T::Index, AccountId = T::AccountId, Call = Call>,
{
    ...
    fn charge_extrinsic_fee(
        transactor: &AccountId,
        encoded_len: usize,
        extrinsic: &<Block::Extrinsic as Checkable<Context>>::Checked,
    ) -> Result {
