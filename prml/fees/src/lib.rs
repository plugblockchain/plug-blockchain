...
use runtime_primitives::{
	...
	traits::{As, CheckedAdd, CheckedMul, CheckedSub, Zero},
};
use support::{
...
pub type CheckedExtrinsicOf<T> =
	CheckedExtrinsic<<T as system::Trait>::AccountId, <T as system::Trait>::Index, <T as Trait>::Call>;
...
impl<T: Trait> MakePayment<T::AccountId, CheckedExtrinsicOf<T>> for Module<T> {
	fn make_payment(transactor: &T::AccountId, encoded_len: usize, _extrinsic: &CheckedExtrinsicOf<T>) -> Result {
