...
> {
	...
	fn send_single_signed_transaction(
		...
		let mut account_nonce = crate::AccountNonce::<T>::get(&account.id);
		debug::native::debug!(
			target: "offchain",
		...
		if res.is_ok() {
			...
			account_nonce += One::one();
			crate::AccountNonce::<T>::insert(&account.id, account_nonce);
		}
...
mod tests {
	...
	type Extrinsic = TestXt<u64, Call, ()>;
