...
pub trait TestNetFactory: Sized {
	...
	fn add_full_peer_with_config(&mut self, config: FullPeerConfig) {
		...
		let block_request_protocol_config = {
			let (handler, protocol_config) = BlockRequestHandler::new(&protocol_id, client.clone(), network_config.default_peers_set.in_peers as usize + network_config.default_peers_set.out_peers as usize);
