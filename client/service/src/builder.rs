...
impl<TBl, TRtApi, TCl, TFchr, TSc, TImpQu, TFprb, TFpp, TExPool, TRpc, Backend>
	...
	pub fn with_import_queue<UImpQu>(
		self,
		builder: impl FnOnce(&Configuration, Arc<TCl>, Option<TSc>, Arc<TExPool>, Option<&Registry>)
			-> Result<UImpQu, Error>
	...
	pub fn with_import_queue_and_fprb<UImpQu, UFprb>(
		...
		builder: impl FnOnce(
			...
			TExPool, TRpc, Backend>, Error>
	where TSc: Clone, TFchr: Clone {
		self.with_import_queue_and_opt_fprb(|cfg, cl, b, f, sc, tx, pr|
			builder(cfg, cl, b, f, sc, tx, pr)
				.map(|(q, f)| (q, Some(f)))
		)
	}
